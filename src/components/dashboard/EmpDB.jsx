...
export default function EmpDB({ onToggleSidebar }) {
  ...
  // --- New State and Functions for Modal Logic ---
  const [modalType, setModalType] = useState(null); // 'checkin', 'checkout', or 'leave'
  ...
  const getQrText = () => {
    if (modalType === 'checkin') return 'CheckIn_' + new Date().toISOString();
    if (modalType === 'checkout') return 'CheckOut_' + new Date().toISOString();
    return '';
  };
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-6 grid-rows-auto gap-4 sm:gap-4 md:gap-6 lg:gap-6 xl:gap-8 p-4 sm:px-4 sm:py-2 md:px-6 md:py-4 lg:px-8 lg:py-6 xl:p-0 xl:pt-8 xl:pl-30 xl:pr-20">
          ...
          <div className="col-span-1 sm:col-span-2 md:col-start-3 md:col-span-4 md:row-start-3 lg:col-start-3 lg:col-span-4 lg:row-start-3 grid grid-cols-3 gap-4 sm:gap-4 md:gap-6 xl:gap-x-8  items-center">
            <button 
              ...
              className="text-[#00B087] text-center text-[14px] sm:text-[16px] md:text-[18px] lg:text-[22px] xl:text-[24px] w-full h-[50px] sm:h-[60px] md:h-[70px] lg:h-[70px] xl:h-[70px] font-bold rounded-3xl bg-[#A6E7D8] ring-4 outline-none ring-[#00B087] "
            >
              Check In
            </button>
            <button 
              onClick={() => setModalType('checkout')}
              className="text-[#DF0404] text-center text-[14px] sm:text-[16px] md:text-[18px] lg:text-[22px] xl:text-[24px] w-full h-[50px] sm:h-[60px] md:h-[70px] lg:h-[70px] xl:h-[70px] font-bold rounded-3xl bg-[#FFC5C5] ring-4 outline-none ring-[#DF0404]"
            >
              Check Out
            </button>
            <button 
              onClick={() => setModalType('leave')}
              className="text-[#928C1F] text-center text-[14px] sm:text-[16px] md:text-[18px] lg:text-[22px] xl:text-[24px] w-full h-[50px] sm:h-[60px] md:h-[70px] lg:h-[70px] xl:h-[70px] font-bold rounded-3xl bg-[#FEF19E] ring-4 outline-none ring-[#928C1F]"
            >
              Leave Request
            </button>
          </div>
  ...
  <Dialog open={true} onClose={() => setModalType(null)} className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.3)] bg-opacity-40">
    <Dialog.Panel className="bg-white p-6 rounded-xl shadow-xl flex flex-col items-center">
      ...
      <div className="p-4 bg-white">
        <QRCode value={getQrText()} size={200} />
      </div>
      <button 
      ...
      {/* Leave Request Modal */}
      {modalType === 'leave' && (
        <Dialog open={modalType === 'leave'} onClose={() => setModalType(null)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl transition-all">
            <Dialog.Title className="text-xl font-bold text-gray-800 mb-6 text-center">
              Request Leave
            </Dialog.Title>
            ...
            <form className="space-y-5">
              ...
              <div>
                ...
                <input
                  type="date"
                  className="w-full rounded-xl border border-gray-300 px-4 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  className="w-full rounded-xl border border-gray-300 px-4 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none resize-none"
